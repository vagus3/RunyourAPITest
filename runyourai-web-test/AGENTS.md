...
- 작업 시작 전 현재 Git branch를 확인한다.
- 반드시 `app/[current-branch]/`를 생성하고 모든 신규 작업물을 해당 디렉터리 안에 생성한다.
- 현재 branch가 아닌 다른 branch의 작업물을 임의로 수정하지 않는다.
- `app/ref/`는 어떤 경우에도 읽거나 수정하지 않는다.
